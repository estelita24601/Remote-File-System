...
- [x] expand the code to allow you to send a command from a client to the server for writing a file from the client's file system to the server.
...
- [x] You need to figure out how to get data from the client to the server.
...
- [x] The server keeps running continuously until its process is killed. The rfs client connects, does its (one) command, closes the socket, and exits, but the server keeps running.
